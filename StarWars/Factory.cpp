...
Gun* BomberFactory::create_gun()
{
	...
	Gun* b_gun = new BomberGun(80, concrete_x, y, angle, w, h, color, r);
	return b_gun;
}
...
SpaceShip* BomberFactory::create_ship()
{
	...
	SpaceShip* b_ship = new BomberShip(600, 2, 2, 3, color, concrete_ship_x, y, angle, r, gun);
	return b_ship;
}
...
{
	int concrete_x = x + diff_between_ships * count;
	Gun* b_gun = new FighterGun(30, concrete_x, y, angle, w, h, color, r);
	return b_gun;
}
...
SpaceShip* FighterFactory::create_ship()
{
	...
	int concrete_ship_x = x + diff_between_ships * count;
	string type = "fighter";
	SpaceShip* f_ship = new FighterShip(400, 2, 5, 1, color, concrete_ship_x, y, angle, r, gun);
	return f_ship;
}
...
{
	int concrete_x = x + diff_between_ships * count;
	Gun* b_gun = new CruiserGun(60, concrete_x, y, angle, w, h, color, r);
	return b_gun;
}
...
SpaceShip* CruiserFactory::create_ship()
{
	Gun* gun = create_gun();
	int concrete_ship_x = x + diff_between_ships * count;
	SpaceShip* c_ship = new CruiserShip(500, 2, 1, 4, color, concrete_ship_x, y, angle, r, gun);
	return c_ship;
}
